 ...
 * place. Build Session 12 Part 5 replaced the direct `ResearchProvider`
 * delegate with `ResearchSynthesisService`, which runs the full grounded
 * pipeline (query planning → retrieval → synthesis → citation
 * validation); this controller's own responsibility stays exactly what
 * it was — entitlement gate, then delegate. The mobile app's
...
export class ResearchController {
  ...
  async query(@CurrentUser() user: AuthenticatedUser, @Body() dto: ResearchQueryDto) {
    ...
    return this.synthesis.answer(dto.query);
  }
}
