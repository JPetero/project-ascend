...
|---|---|---|---|---|---|
| 1 | Android | Small/low-end phone (e.g. a budget device, ~4-6GB RAM) | Minimum supported API level — API 26, confirmed to match `android/app/build.gradle.kts`'s actual `minSdk` (S14 Part 4 explicitly set it to 26 there, overriding Flutter's lower template default, after a real GitHub Actions run showed the `health` plugin's manifest requires it) | Oldest-OS + weakest-hardware combination is where camera/ML Kit performance (Vision) and background cardio tracking are most likely to degrade or crash | NOT_RUN |
...
| 28 | Release build (not debug) — obfuscation/signing configured, no debug banners or `flutter run` dev tooling visible | NOT_RUN | S13 Part 16-27 wired a real `key.properties`-driven signing config into `android/app/build.gradle.kts` (see `android/key.properties.example`); S14 Part 1 changed the `prod` flavor specifically to hard-fail `assembleProdRelease`/`bundleProdRelease` when no real signing is configured, rather than silently falling back to debug signing — so a `prod`-flavor artifact can no longer exist without real signing. This row still can't pass until a real upload keystore is actually generated and populated (locally via `key.properties`, or in CI via the `ASCEND_KEYSTORE_BASE64`+friends secrets — see `founder-setup-checklist.md`) and a resulting `prod`-flavor release build is produced and confirmed on real hardware. |
